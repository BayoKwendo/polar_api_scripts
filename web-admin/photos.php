        ...
        <div class="card-header">
          <h5>
            ...
            <span class="pull-right">
              <a class="btn btn-sm btn-primary" data-id="-1" data-toggle="modal1" data-target="#photoModal1" href="photos.php?pg=-1" ><i class="fa fa-page"> Upload Photo</i></a>
              &nbsp;|&nbsp;
              <a class="btn btn-sm btn-success" href="photos.php" >View All</a>
            </span>
          </h5>
        </div>
        <div class="card-body">
          <div class="table-responsive">
            <?php
              if(isset($_REQUEST['pg'])){
                ...
                $pg = $id>0? db_get_row("SELECT * FROM photos WHERE id='$id' "):FALSE;
                //var_dump($pg);
                ?>
                <form method="post" action="formSubmit.php" enctype="multipart/form-data" >
                    ...
                    <div class="modal-body">        
                          <legend>Upload Photo Here</legend>
                            ...
                            <?php } ?>
                                <div class="col-md-6 col-12">
                                    <div class="form-group">
                                        <label for="file" >Browse Photo File</label>
                    ...
                    <div class="modal-footer">
                        <div class="left-side">
                            <input type="submit" class="btn btn-info btn-sm " value="Save" name="photo_entry" />
                        </div>
                    </div>
                </form>
                <?php
                ...
                echo '<tbody>';
                if($pages = db_get_all("SELECT * FROM photos ORDER BY caption ASC ")){
                  //var_dump($pages);
                  $count = 0;
                  foreach($pages AS $row){
                    $link_view = '<a href="#" data-id="'.$row->id.'"  data-caption="'.$row->caption.'" data-toggle="modal" data-target="#photoViewModal" >'.$row->caption.'</a>';
                    $btn_edit = '<a href="photos.php?pg='.$row->id.'"  class="btn btn-sm btn-warning" data-id="'.$row->id.'"  data-caption="'.$row->caption.'" data-toggle="modal1" data-target="#photoModal1" ><i class="fa fa-upload"></i></a>';
                    $del_msg = "Records for Photo: $row->caption ";
                    $btn_del = '<button class="btn btn-sm btn-danger" data-id="'.$row->id.'"  data-tbl="photos" data-msg="'.$del_msg.'"  data-toggle="modal" data-target="#deleteModal" ><i class="fa fa-trash"></i></button>';
...
<?php include ('footer.php'); ?>
  ...
  <!-- Page View Modal -->
  <div data-backdrop="true" class="modal fade " id="photoViewModal" tabindex="-1" role="dialog" aria-labelledby="photoViewModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        <div class="modal-header">
          <h4 class="modal-title" id="photoViewModalLabel"></h4>
  ...
  <script>
    $('#photoViewModal').on('show.bs.modal', function (event) {
        ...
        var modal = $(this);
        $.post("formSubmit.php", {'ajax-photo-view': id}, function (data) {
            //alert(data);
